...
- [ ] **Poetry Unit 1: GitHub push** — Commit `build_poetry_student_packet.py`, `build_poetry_cap.py`, `build_poetry_session_tracker.py`, `build_poetry_unit1.js`, `_Operations/cbd_docx_template.js` (productLine param + rootKey fix + adm-zip strip). Open GitHub Desktop → commit → push. *(Updated 2026-04-03)*
- [ ] **Poetry Unit 1: Re-export COMPLETE.pdf from Word** ⚠️ **Jill action required** — Open `What_the_Voice_Carries_COMPLETE.docx` in Word → File → Save As → PDF. Existing PDF was LibreOffice (violates CbD rule). Do this before TPT listing. *(Added 2026-04-03)*
- [ ] **Poetry Unit 1: QC sign-off after Word re-export** — Once PDF is re-exported, verify cover reads "A Poetry Reading Unit" → then update Airtable Workflow Stage from "Building" to "Ready for TPT." *(Added 2026-04-03)*
- [ ] **Poetry Unit 1: TPT listing package** — Output 0: title ≤80 chars, description opener ≤180 chars, 7 keyword tags. Build after Canva cover exported. *(Added 2026-04-02)*
- [ ] **Poetry Unit 1: Export Canva cover + Reel** — Cover: `DAHFebI7Z2k` (export PNG). Reel: `DAHFutJbS0o` (finalize draft → export). Upload cover to TPT at listing time. *(Added 2026-04-02)*
...
- [ ] **Substack Post #5: FB group drop** — Drop link in 2–3 active SPED Facebook groups where facilitated spelling / AAC skepticism is being discussed. Comment: *"I actually wrote about this — [link]. The short version: we say we presume competence, but we don't extend that same belief to every communicator — and our kids know it."* *(Added 2026-04-10)*

...
| Element | Value |
|---------|-------|
| **Tagline** | Where SDI & AT Meets Practice |
...
| **PB Companion Image 2 template** | DAHHIU6OBVI | 1080×1080 — 1 page. 2 image placeholders (CommBoard + StudentActivities) + book_title text variable. Bulk import via `PB_Companions_Canva_BulkImport.csv`. |
| **PB Companion Image 3 template** | DAHHIb2bM_A | 1080×1080 — 1 page. 3 image placeholders (StudentActivities p.1 + StudentActivities p.2 + Partner Info). Pending partner info PNG export. |

→ Full pin design index + workflow: `Marketing/Pinterest/CBD_Canva_Pin_Design_Index.md`
...
|------|-----------------|----------|
| **TPT store** | **23 products LIVE (Apr 10 2026).** AT/AAC IEP Team (7) · Nonfiction (6) · Picture Book Companions (6) · Bundles (2) · Fiction (1) · Poetry (1). UFLI (36 products) is next major launch. | May 2026 |
| **All the Way to the Top Companion** | K–3 picture book companion. Price: $5. **LIVE on TPT (Apr 4 2026).** TPT ID: 15979429. Airtable: `recPx1oIQOMEUBP9E` (Live ✓). Vocab synced ✓ (Apr 10 2026). File set: Welcome (2pp) · Teacher Packet (9pp) · Student COMPLETE (12pp assembled) · Symbol Pages (2pp) · TPT Preview (10pp). Jennifer Keelan bundle: pair with Capitol Crawl for Disability Pride Month July 2026. | July 2026 (ADA anniversary) |
...
| **Substack** | **5 posts live (Apr 10 2026).** Post #5 "Who Gets to Decide If the Voice Is Real?" published April 10, 2026. URL: https://communicatebydesign.substack.com/p/who-gets-to-decide-if-the-voice-is. Pillar: 🔴 Policy & Advocacy. **Next: Post #6 "Auditory Confirmation Loop" — Tue Apr 14 (scheduled task `cbd-substack-auditory-confirmation-loop` fires 9AM).** Gold standard post format locked: `Substack/CbD_Substack_Post_Format_GoldStandard.md`. Tailwind distribution package now included with every post (strategy locked Apr 10 2026). **⚠️ FB group drop for Post #5 pending — do today.** | Apr 2026 |
| **Exceptional Children's Week** | **Apr 13–17, 2026.** Theme: "Turn the Page to Possibility: Literacy Success in Special Education." Full 5-day content plan built (Apr 11 2026): 15 IG Story slides (3/day) + carousel concepts + FB posts + FB Group drops for every day. Files: `Marketing/EC_Week_2026_Content_Plan.md` · `Marketing/EC_Week_2026_Story_Slides.md`. Day map: Mon=access design thesis (V1/V2/V3) · Tue=AT+Substack Post #6 (highest leverage day) · Wed=disability rep in fiction+PB · Thu=nonfiction diverse voices · Fri=community reading+Substack CTA. **⚠️ All 5 days need IG carousel graphics built in Canva (Jill) + Story slides built + FB posts scheduled.** | Apr 13–17 2026 |
| **Re-upload nonfiction zips** | All 6 units need re-uploaded to TPT — Session 16 content update + Apr 2026 file architecture standardization (new Student Print Materials .docx + updated Welcome PDF). Jill: delete stale files from each `_TPT/` folder first, then zip 4 files only. | — |
...
| **Symbol Card Sheet Generator** | Standalone browser tool, Session 26 (Sep 2026). **Merged to `main` 2026-09-25; routine mode added 2026-09-26.** Three modes in one file: (1) **Print symbol cards** — type words → ARASAAC symbols → print at true size (1"–3", default 1.5", cutting space between cards), multi-word fallback ("back stretch" → stretch), Fitzgerald borders, Google Docs export. (2) **First–Then board** — First/Then · Now/Next · Now/Next/Then or custom labels; prints for laminate+velcro OR runs on a device with speech; optional student choice of what comes next. (3) **Routine & timers** — up to 10 steps, any step can be a student choice, optional minutes per step; device view shows the whole strip so the student sees what is coming; printed strip page 1, cut-out velcro cards page 2 (steps sharing options share one card set). **Any box in modes 2–3 holds up to 3 pictures** — `calm, body` reads as ONE message, not two steps. **Timer rule (locked):** the clock never advances the routine and no duration is ever logged — how long a student needed is not data about the student. One shared **choice log** across both boards measuring language growth (acts, distinct words, spontaneous vs prompted, honored vs could-not) — no accuracy figure anywhere, CSV export, stays on the device. Directions sit at the bottom of the page. File: `_Operations/Tools/cbd-pecs-sheet-generator.html`; hosted copy generated by `_Operations/Tools/build_deploy.py` (never edit `deploy/`). **Working and classroom-ready. NOT an official product yet** — 5 open decisions (name/PECS trademark, no links out, no end matter, distribution, support burden). Full spec incl. routine mode, timers, log, and the print/layout bugs worth remembering: `_Operations/memory/symbol_card_tool_reference.md`. Drive folder: "Symbol Card Sheets" (private). | Phase 0 pending |
...
| **Canva Image 1 Share URL** | **`fldYDDTNbdlE8AfNv`** | URL | **NEW 2026-04-17.** Canva view/share link for Image 1 (cover/pin graphic). Paste after bulk import. Feeds Tailwind CSV. |
| **Canva Image 2 Share URL** | **`fldwAZkqxIEBReiNp`** | URL | **NEW 2026-04-17.** Canva view/share link for Image 2 (CommBoard for PB Companions; CAP cover for Nonfiction). Feeds Tailwind CSV. |
| **Canva Image 3 Share URL** | **`fldK2TRQ7xkodA4w8`** | URL | **NEW 2026-04-17.** Canva view/share link for Image 3 (Student Activities). Feeds Tailwind CSV. |
...
**Tailwind for Pinterest (exploration started 2026-04-08):** Jill is evaluating Tailwind (tailwindapp.com) as a Pinterest scheduling/distribution tool. The 4 Pinterest metadata fields in Airtable (Pin Title, Description, Alt Text, Canva Media URL) are structured to feed this workflow. Status: exploration — no workflow locked yet. Do not assume Tailwind is active or build Tailwind automation without Jill confirming.

**TPT Discounts (active 2026-04-08):** 10% new follower discount (ongoing). 10% Spring Sale — one week.
...
| Pinterest Daily Brief | `cbd-pinterest-daily-brief` ✅ | 8 AM daily (auto) | Reads Products table → generates 3 pin briefs (2 List A first pins + 1 List B fresh pin) → saves to `Marketing/Pinterest/Daily Briefs/` → no Pinterest automation. **USE THIS ONE.** |
| Pinterest v3 (retired) | `cbd-daily-pinterest` 🚫 | Disabled | Superseded by cbd-pinterest-daily-brief. Do not re-enable. |
| Dashboard Refresh | `cbd-dashboard-refresh` | 3×/day (8:16 AM, noon, 11:30 PM) | Regenerates all 4 dashboards. **CRITICAL: HTML edits are overwritten — all data corrections must go in this task's prompt.** Updated Apr 5 2026: PB Companion row hardcoded (vocab counts not from Airtable — not synced yet); Poetry Unit 1 shows 28 words "(not synced)"; Core/Fringe/Heart columns show word counts not checkmarks. |
...
| **Nonfiction Reading Units** | — | 6 live + 1 freebie (Keiko Part 1). Grades 6–10. **Pipeline Units 7–10 (Apr–Sep 2026 cadence, 1/month paired with fiction):** Unit 7 = Fred Korematsu (May, APAHM, Skill: Point of View/Perspective, pairs with The Giver) · Unit 8 = Judy Heumann (Jun, disability identity/cause-effect, pairs with Rules; cross-catalog funnel to 504 bundle) · Unit 9 = Malala Yousafzai (Aug, survival/CER applied to speech, back-to-school hook, pairs with Hatchet) · Unit 10 = Roberto Clemente (Sep, Hispanic Heritage Month, Skill: Compare/Contrast — FIRST compare/contrast unit in catalog, pairs with Ghost). |
| **Fiction Anchor Texts** | — | **1 live (Apr 2026): Wonder — Fiction Anchor Text Unit. LIVE on TPT. TPT ID: 15945146. Price: $6.00. Airtable: `reccsVcc4bkuaiIJ9`.** SDI companion units for traditional K–12 novels already in classrooms. NOT a Lexile differentiation product — text stays the same, access layer is the product. Skills target RL standards. Framework: Story Grammar (Spencer & Petersen 2020) + Description-First activity design + Visual Scene Displays + Emotional/mental state vocabulary pre-teaching. Product architecture: two files per unit — main unit docx + Fiction Printable Packet PDF (Python/ReportLab, 4 layers, 9 pages). Two vocabulary sets: Set A = SDI instruction targets (symbol cards + Vocab Map), Set B = full communication board vocabulary. Printable Packet spec: `_Operations/memory/fiction_printable_packet_spec.md`. Reference: `_Operations/memory/fiction_reference.md`. Research: `Research/Fiction_Narrative_Research_Foundation.md`. Brand color TBD. Pricing model TBD (not passage-count based). **5-IMAGE MARKETING STANDARD (locked Apr 17 2026):** Image 1 = Cover (Canva bulk template `DAHGBZ-LtRo` — Jill exports) · Image 2 = Communication Board A (p.4 of Printable Packet, idx 3) · Image 3 = Core Word Symbol Cards (p.2, idx 1) · Image 4 = Partner/Communication Environment Setup (p.1, idx 0) · Image 5 = Student Activity Part 1 (p.9, idx 8) · Image 6 = Student Activity Part 2 (p.10, idx 9). All PNGs exported to per-unit `Marketing/Images/` folder (NOT shared folder). Export script: `_Operations/Build/export_fiction_marketing_images.py` — outputs to `[Unit Folder]/Marketing/Images/`. Wonder symbol fix (Apr 17 2026): `SYM_CACHE` path corrected to `_Operations/Symbols/symbol_cache`; `young`→`arasaac_little.png` and `hides`→`arasaac_away.png` substitutions added. **TPT PREVIEW PDF (locked Apr 17 2026):** Required BEFORE creating any TPT listing. Script: `_Operations/Build/build_fiction_previews.py`. Source: Printable Packet PDF from `Product Files/` (always available — no Word export needed; this is the key difference from nonfiction which needs a Word export). Preview = 8 pages: branded cover + partner setup + symbol cards + Board A + Board C + Part 1 student page + Part 5 synthesis + branded back. Output: `TPT Product Files/[Unit]_TPT_Preview.pdf` — lives with upload files ONLY, no central copy. Wonder ✓ built Apr 17 2026. The Giver ✓ built Apr 17 2026. **FOLDER STRUCTURE (locked 2026-04-17):** Every fiction unit uses a consistent 4-folder layout — no files scattered elsewhere. `Builds/` (all build scripts for this unit) · `Product Files/` (source-of-truth: Teaching_Materials.docx + Printable_Packet.pdf + Welcome.pdf) · `TPT Product Files/` (4 upload-ready files + preview + zip) · `Marketing/` (Marketing_Plan.md + Canva CSV + Tailwind CSV + Images/ subfolder with 5 PNGs). **Marketing_Plan.md is the single file you open for ANY copy/paste task** — TPT title/description/tags, all 3 Pinterest pins with title+description+alt text, Instagram caption, Facebook group drop, seasonal hooks, bundle strategy. Canva CSV is per-unit in Marketing/ — NOT in Marketing/Canva/ or any shared folder. Images are per-unit in Marketing/Images/ — NOT in a shared Marketing/Product Images/Fiction Anchor Texts/ folder. Full spec: `_Operations/memory/fiction_printable_packet_spec.md`. **Wonder marketing plan:** `Products/Fiction Anchor Texts/Wonder - Character Analysis/Marketing/Wonder_Marketing_Plan.md`. **The Giver marketing plan:** `Products/Fiction Anchor Texts/The Giver - Theme Analysis/Marketing/The_Giver_Marketing_Plan.md`. **Rules marketing plan:** `Products/Fiction Anchor Texts/Rules - Identity and Belonging/Marketing/Rules_Marketing_Plan.md`. ⚠️ Jill: delete stale files from Finder — old `[Unit]_TPT/` folders, old `_TPT_Listing_Package.md` files in unit roots, loose build scripts in unit roots, shared `Marketing/Product Images/Fiction Anchor Texts/` folder, any `Fiction_Canva_BulkImport.csv` or `Fiction_TPT_BulkImport.csv` in shared Distribution/ folders, `Products/Fiction Anchor Texts/Preview PDFs/` folder (all moved to per-unit structure). **iCloud write rule for build scripts:** write preview/image output to `/tmp/[filename]` first, then `shutil.copy2()` to iCloud destination — cannot overwrite existing iCloud files directly from sandbox. **Pipeline: 31 titles in Airtable Launch Pipeline. 11 of 31 are disability-representation texts (AAC, autism, physical disability, learning differences, hearing loss, dyslexia). This is a brand identity, not a coincidence.** **CONFIRMED 6-UNIT LAUNCH SET (Session 22):** 1→Wonder (active, Gr 3–8, RL.5.3/5.6, Airtable: `reccsVcc4bkuaiIJ9`) · 2→The Giver (Gr 6–8, RL.7.1/7.2, Building) · 3→Rules: Identity and Belonging (Gr 4–6, RL.4.3/4.6/5.3, Building, Airtable: `recnwj9W2F99YBmzb` — ALL FILES BUILT Apr 18 2026: Teaching_Materials.docx ✓ · Printable_Packet.pdf 13pp ✓ · Welcome_to_the_Product.pdf 2pp ✓ · TPT_Preview.pdf 8pp ✓ · Marketing_Plan.md ✓ · Vocab in cbd_unit_vocab.js ✓. ⚠️ Jill manual steps: run export_docx_to_pdf.py · Canva cover export from DAHGBZ-LtRo · run export_fiction_marketing_images.py --title "Rules" · zip 4 files · list on TPT) · 4→Out of My Mind (Gr 4–7, RL.5.3/5.6, AAC protagonist ⭐ flagship) · 5→Hatchet (Gr 5–7, RL.5.2/5.5) · 6→Ghost (Gr 5–8, RL.7.1/7.3, BHM hook). All 6 records tagged in Airtable as "🚀 LAUNCH SET — Unit X of 6". |
| **Picture Book Companions** | K–3 | **LINE POSITIONING (locked Apr 12 2026):** AAC-inclusive K–3 read-aloud companions with whole-class evidence-based activities (IRA + Dialogic Reading + ALS). Disability representation is a feature of the launch set — NOT the defining identity of the line. Going forward, titles will span many themes beyond disability rep. Marketing frame: "built for the whole class, with communication access designed in from the start — not added on." Do NOT lead with disability rep in descriptions or social copy for future titles. **1 live (Apr 4 2026). 5 built (Apr 4 2026) — QC visual pass still needed in Word before TPT listing.** First product: All the Way to the Top — **LIVE on TPT. TPT ID: 15979429. Airtable: `recPx1oIQOMEUBP9E`.** Framework: IRA + Dialogic Reading + ALS. Research: `Research/Picture_Book_Companions_Research_Foundation.md`. Pipeline research: `Research/CbD_PictureBook_Pipeline_Research.md` (28 titles, SEO analysis, priority scoring). Standards: RL.K-3.3 + RL.K-3.4. 3-reading structure. ALS one-pager embedded. Vocab ceiling: 15–20 words. Color: Dusty Rose-Coral #D4614E (locked Apr 3 2026). Symbol pages and comm board: Python/ReportLab ONLY (never inside .js build). PDF: LibreOffice headless (verified clean Apr 4 2026 — NOT Word export). **3-FILE BUILD ARCHITECTURE (locked Apr 4 2026):** (1) Teacher Packet (~12pp) — COMPLETE reference: standards, frameworks, comm access, vocab routine w/ARASAAC symbols, partner scripts with all 3 readings, activities answer key, accessibility/about/terms. (2) Teacher Quick Start (~4pp) — print-and-go: how-to callout + vocab routine + partner scripts + answer key. No frameworks, no standards tables. (3) Student Activities — activities + sentence frames + student reading tracker (3-reading table: date, participation checkboxes, thoughts column; "My favorite part" write lines) + comm board instructions. Student COMPLETE assembled by `build_student_complete.py`: Student Activities + Comm Board + Symbol Pages + AAC Session Tracker = ~11pp. **All 5 titles (Apr 5 2026):** A Friend for Henry (K–1, RL.K.3–RL.1.3, belonging/autism) · I Talk Like a River (K–2, RL.K.3–RL.2.3, communication identity) · Ian's Walk (K–1, RL.K.3–RL.1.3, perspective/sensory) · Emmanuel's Dream (1–2, RL.1.3+RI.1.3, biography/persistence) · My Friend Isabelle (K–1, RL.K.3–RL.1.3, friendship/Down syndrome). **Vocabulary (Apr 5 2026):** All 6 PB Companion titles fully populated in `_Operations/Build/cbd_unit_vocab.js` — pendingBuild removed, two-dimension word lists complete (~112 words across 6 titles). 5 title mappings added to `sync_vocab_to_airtable.js` UNIT_TITLE_MAP. ⚠️ Jill action: run `sync_vocab_to_airtable.js` to add ~140 new records to Airtable (PB Companion + Poetry Unit 1). SEO gap confirmed: zero TPT competition for all 5 titles. Build scripts in each title's folder under `Products/Picture Book Companions/`. **Canva bulk import CSV:** `Marketing/Canva/PB_Companions_Canva_BulkImport.csv` (or per-title Marketing/ folder) — 6 rows (all companions), 10 columns (added image2_filename + image3_filename Apr 17 2026). **Canva cover workflow (locked Apr 4 2026):** Bulk template design ID = `DAHF6DObHZ4` ("(Bulk 1) Picture Book Square Template", 6 pages, 1080×1080). Text variables (title, standards, skill focus) auto-populate via bulk CSV — Lane B. Image placement (book cover photo + symbol preview + 3 worksheet previews = 5 images per title) = Lane A, Jill places manually in Canva editor. **3-IMAGE MARKETING STANDARD (locked Apr 17 2026):** Image 1 = Cover (Canva `DAHF6DObHZ4`) · Image 2 = Communication Board (p.1 of `[Key]_Communication_Board.pdf`) · Image 3 = Student Activities p.1 — Activity 1 (from `[Key]_Student_Activities.pdf`). All 12 PNGs exported and saved to `Marketing/Product Images/PB Companions/`. Export script: `_Operations/Build/export_pb_marketing_images.py`. **Canva Image 2 template: `DAHHIU6OBVI`** (2 image slots + book_title — bulk import ready, all 6 CommBoard PNGs uploaded to Canva). **Canva Image 3 template: `DAHHIb2bM_A`** (3 image slots — pending partner info PNG for slot 3). **Canva bulk import image rule:** images must be uploaded to Canva media library first; CSV references exact upload filename. Future expansion: Partner Script page (Image 4) and Vocabulary Preview Routine (Image 5) are strong candidates — decision pending. **TPT listing fields:** Airtable Products table has TPT metadata fields (Product Type, Tax Code, Grade Levels, Resource Types, Duration, Answer Key, Page Count, Custom Tags, Subject Areas) — all 5 titles populated Apr 5 2026. ⚠️ TPT custom tags: account-wide limit is 20 slots for the ENTIRE store (not per product). Current per-product tags in Airtable are placeholders — needs master 20-tag store strategy before going live. |
| **UFLI Phonics** | — | 36 products built, not yet listed. Target May 2026. |
| **Poetry Reading Units** | — | **1 live (Apr 10 2026).** NEW product line est. 2026-03-30. **Architecture: access-layer model** (same poem across V1/V2/V3 — NOT text-versioned). CbD-original strategy: **NOTICE/FEEL/MEAN/ASK (NFMA)**. Price: $9.95/unit. **4-UNIT LINE LOCKED (Apr 10 2026) — grade bands and standards finalized:** Unit 1 = What the Voice Carries (Gr 5–7, RL.5.4/6.4/7.4, L.5.5a–c, figurative language, Apr 2026) · Unit 2 = Who Is Speaking (Gr 6–8, RL.6.6/7.6/8.6, speaker/POV, Jul 2026) · Unit 3 = How Form Shapes Feeling (Gr 7–9, RL.6.5/7.5/8.5, structure, Sep 2026) · Unit 4 = What We Share (Gr 8–10, RL.7.2/8.2/9-10.2/9-10.9, theme+synthesis, Nov 2026). Grade band logic: each unit steps up one band (5–7→6–8→7–9→8–10). Standards and grade band always match — a 10th grade teacher using Unit 1 is doing SDI, not grade-level instruction. **Unit 1 "What the Voice Carries" status: LIVE on TPT (Apr 10 2026).** URL: https://www.teacherspayteachers.com/Product/What-the-Voice-Carries-Poetry-Figurative-Language-Adapted-ELA-SPED-5-7th-16037413 · TPT Product ID: 16037413 · Price: $9.95. FILE SET: COMPLETE.docx · COMPLETE.pdf (Word-gen, 24pp) · Student_Packet.pdf (13pp) · CAP.pdf (12pp) · Welcome_to_the_Product.pdf (4pp). Airtable Products record: `receS8L09qmIxUmcO` (Gr 5–7, all milestones ✓, Workflow Stage: Live). Launch Pipeline: `recgA5kflO2nGrjmB`. Canva cover: `DAHFebI7Z2k`. TPT Listing Package: `Unit1_TPT_Listing_Package.md` (Output 0 updated to grades 5–7 ✓ + Tailwind distribution package). Substack draft ready: `Substack/Substack_Draft_WhenTheFrameworkDemandsAConfession.md`. **⚠️ Next: Pinterest 3 pins + Instagram carousel + Facebook post per Tailwind package.** **CRITICAL BUILD RULES (fixed Apr 3 — apply to ALL future build scripts):** (1) Import `{Paragraph, HeadingLevel, etc.}` from `T` not `require("docx")` — different instances break headings/page breaks. (2) `assembleAndWrite()` auto-strips rootKey XML tags via adm-zip (`_Operations/node_modules/adm-zip` required). (3) `titlePage()` accepts `opts.productLine` — pass `"A Poetry Reading Unit"` for poetry line. Initial build plan: `Research/CbD_Poetry_Unit_Line_Initial_Build_Plan.md`. |
...
- Pillar: 🔴 Policy & Advocacy. Topic: facilitated spelling skepticism, presuming competence of student AND partner, ASL interpreter analogy, partner training standards needed. Personal anchor: family experience of an AAC user's communication being doubted by adults and peers. *Out of My Mind* Whiz Kids scene. Draft saved: `Substack/Substack_Draft_WhoGetsToDecide_Voice.md`
...
**Substack priority:** #2 after TPT product lines. **Goal: 200 free subscribers to unlock paid monthly subscriber feature.** Every post and FB group drop should drive toward this threshold. Current count: unknown — check Substack dashboard.
**Paid subscription tier (locked Apr 10 2026 — activate at 200 free subscribers):** Suggested $8/month or $80/year. Free tier = all 5-pillar posts (public). Paid tier = (1) monthly resource drop — 1 classroom-ready tool/template built from existing product material; (2) early TPT access — 48hr preview link before any product goes live; (3) monthly Q&A thread — Jill answers 5 subscriber questions. Do NOT activate paid tier before 200 free subscribers.
**Upcoming Substack post (drafted Apr 10 2026):** "When the Framework Demands a Confession" — 🔵 Instructional Practice, ~820 words. TPCASTT vs. NFMA argument. References Science of Reading Classroom post (Bob Sparks, Apr 8 2026) as peer dialogue. FB drop target: threads about poetry for students with disabilities. **Draft saved: `Substack/Substack_Draft_WhenTheFrameworkDemandsAConfession.md`. Ready for Jill review.**
→ Full pipeline + growth workflow + subscription tier detail: `Substack/CbD_Substack_Growth_Workflow.md`
...
> ☐ READ `cbd_worksheet_templates.py` first — check every existing template before designing any new activity page. If the layout you need already exists, use it. Do NOT create a new canvas function or custom layout without first confirming it isn't already in the template system.
> ☐ Student worksheet pages will be built with `cbd_worksheet_templates.py` — NOT inside the .js docx build

**Available templates (check this list before designing anything new):**
`make_mcq_page()` · `make_short_answer_page()` · `make_cer_page()` · `make_evidence_sort_page()` · `make_vocab_preview_page()` · `make_annotation_guide_page()` · `make_descriptor_board_page()` · `make_before_after_page()` · `make_partner_prompt_card()`

If a needed layout genuinely isn't covered → add it to `cbd_worksheet_templates.py` as a new named template function, then use it. Never build one-off inline layouts.
...
**`cbd_worksheet_templates.py` handles:** all student response pages (MCQ, short answer, CER, evidence sort, vocab preview, annotation guide, before/after, descriptor board).
...
## Fiction Line — CRITICAL OVERRIDE (skill reference file is wrong — this overrides it)
**⚠️ The `communicate-by-design` skill file at `references/fiction.md` contains INCORRECT information about the fiction line. Ignore anything in that file about V1/V2/V3 or Lexile ranges for fiction. The authoritative source is `_Operations/memory/fiction_reference.md`.**

**Fiction line — non-negotiable rules that override any other source:**
- **NO V1/V2/V3 text versions in fiction.** This was never part of the fiction line.
- **NO Lexile ranges for fiction.** We do not rewrite the novel. The text is the original published book.
- **Differentiation in fiction = access layer ONLY:** AAC tools (symbol cards, communication boards, partner guidance), response scaffolds (sentence frames, symbol sorts, Describe to Draw), partner mode, vocabulary pre-teaching.
- **Every student reads the same text.** Every student works toward the same standard.
- The fiction line was built from its own research foundation (`Fiction_Narrative_Research_Foundation.md`) — do NOT apply nonfiction build rules to fiction.
- "What This Unit Is Not" in every fiction unit explicitly states: "A simplified or rewritten version of the novel"
- About the Creator + Terms of Use + Accessibility Statement → Welcome PDF only, NOT in Teaching Materials docx.
- Before any fiction build: read `_Operations/memory/fiction_reference.md` in full. Do not rely on the skill reference.

## Printable Materials Build Rules — LOCKED 2026-04-17
These apply to ALL ReportLab-built PDFs (fiction packets, nonfiction CAP, picture book symbol pages, poetry packets). No exceptions.

**Color:**
- FK color = border only on all cells, chips, and cards. Never FK background fill.
- Section header bars: white fill + colored left accent bar (3pt) + navy text. Never solid fill + white reversed text.
- Column headers (Before/After, T-chart): white fill + colored border + navy text.

**Response/writing areas:**
- No fill where students write. White only — no gray (#F8FAFC), no tinted wash (#FFFBEB).
- Structure = border + navy left accent bar. Fill = never.

**Prompt language:**
- Prompts ask the literary/content question only.
- Never narrate the page: no "use the sentence frame," "use the vocabulary strip," "use the starters below," "write or draw your ideas in the chart."
- The page shows the affordances. The prompt asks the question.

**Student info row:**
- Fields: Name · Class · Teacher · Date. Four fields. White fill. Full width.
- Never: "Name / AAC User," "AAC Student," or any disability-identifying label in a field name.
- Every student in the class fills in the same four fields. The worksheet does not signal who it was made for.

**AAC access language on all pages:**
- Core → `"Core — part of most AAC ecosystems"`
- Fringe → `"Fringe — SDI target; include in communication packet"`
- Never: "confirm with AAC team," "consult the team," "check with your SLP," "coordinate with" on any student-facing or vocabulary-facing content.
- Team direction belongs in teacher guidance sections only.

---

## Hard Rules (Never Violate)
- **CLAUDE.md is a map, not a filing cabinet.** Product-specific copy (TPT title/description, Pinterest pin text, social posts, Canva IDs, seasonal hooks) belongs in `[Product Folder]/Marketing/[Unit]_Marketing_Plan.md` — NOT in CLAUDE.md. CLAUDE.md documents the rule and the path, never the content itself.
- **Everything for a product lives inside that product's folder.** No files scattered across shared Distribution/ or Marketing/ folders. The only exception is shared build scripts in `_Operations/Build/` that are used across multiple products.
- **QC rule — two types, two locations:** (1) Shared QC templates (checklists, rubrics that apply to a whole product line) → `_Operations/QC/`. (2) Per-product QC records (diagnostic evaluations, content extraction files, QC notes for a single unit) → that unit's `Product Files/`. Never put a per-product QC file in `_Operations/QC/`.
...
- **PDF generation:** Run `python3 _Operations/Build/export_docx_to_pdf.py path/to/file.docx` — calls Word's own engine via osascript/docx2pdf. NEVER LibreOffice for nonfiction/poetry/fiction. **Exception: Picture Book Companions** use LibreOffice headless conversion (verified clean Apr 4 2026 — fonts and layout intact). The old "Word → File → Save As → PDF" manual step is retired — use the script.
- **Symbol pages for picture book companions:** Python/ReportLab ONLY (`build_symbol_pages_picbook.py`). NEVER build symbol pages inside a `.js` docx build. Cards: symbol image + ALL CAPS word label + Fitzgerald Key colored border ONLY — no category bar, no star, no POS label, no extra text. Card size: 2"×2", 3-col × 4-row grid, 3pt FK border. Two sections: Core Words + Fringe Words — Unit-Specific Vocabulary (never merged). Root cause of Apr 2026 failures documented in `_Operations/memory/build_system_reference.md`.
...
- **Automated PDF export (locked Apr 10 2026):** `_Operations/Build/export_docx_to_pdf.py` — calls Word engine via docx2pdf/osascript; identical to File → Save As → PDF; NEVER LibreOffice for nonfiction/poetry/fiction. Usage: `python3 _Operations/Build/export_docx_to_pdf.py --product-line poetry|nonfiction|fiction`. Install once: `bash _Operations/Build/install_cbd_tools.sh`. PATH note: add `export PATH=$HOME/bin:$PATH` to `~/.zprofile` for persistent `cbd-pdf` command.
- **Poetry Welcome PDF:** `build_welcome_poetry_unit1.py` in unit folder — ReportLab, Deep Violet brand, 4pp. Run from unit folder or with full path. Output: `WhatTheVoiceCarries_Welcome_to_the_Product.pdf`.
- **Communication Access Packet (multi-unit):** `_Operations/Build/build_all_units.py` — call `build_comm_access_packet()` ONLY. **NEVER call `assemble_tpt_folder()`** — it is outdated, overwrites the welcome PDF, and causes SameFileError. Expects COMPLETE PDFs in scratch `/sessions/[id]/` with expected filenames (see `build_system_reference.md`). Word lists + page counts in `nonfiction_build_reference.md`.
- **Communication Access Packet (single-unit reference):** `_Operations/Build/build_comm_access_packet.py` (Frances Kelsey)
- **Welcome PDF (nonfiction):** `_Operations/Build/build_welcome_nonfiction.py` — reads `*_DRAFT.md`, builds 3 intro pages + Teacher AK pages. **Run this LAST — always after CAP build.** Page counts: Frances Kelsey=11pp, Radium Girls=8pp, Keiko=7pp, Zitkala-Sa=4pp, 504 Sit-In=7pp, Capitol Crawl=4pp. → outputs `[Unit]_TPT/[Unit]_Welcome_to_the_Product.pdf`
- **Nonfiction TPT folder = 4 files (locked Apr 2026):** `[Unit]_COMPLETE.docx` · `[Unit]_Student_Print_Materials.docx` · `[Unit]_Welcome_to_the_Product.pdf` · `[Unit]_Communication_Access_Packet.pdf`. Zip only these 4 for upload. Delete stale files first (duplicates, lock files, old PDFs).
- **Student Print Materials:** Built by `build_*.js` as second output — student handouts + all 3 version packets, NO teacher content, NO answer key. Uses index-marker pattern (`handoutStart`, `v1Start`, `v2Start`, `v3Start`, `akStart`) to slice children array.
...
├── Marketing/                   ← ALL marketing assets live here now (moved Apr 17 2026)
│   ├── Campaigns/               ← time-boxed campaign content (e.g. April EC Week)
│   │   └── April EC Week/
│   ├── Canva/                   ← Canva exports + templates not tied to a specific product
│   ├── Fonts/                   ← brand fonts (Arial Rounded MT, Helvetica Now)
│   ├── Instagram/               ← Instagram-specific assets
│   ├── Photos/                  ← product + lifestyle photos
│   ├── Pinterest/               ← Pinterest strategy docs + daily brief outputs
...
│   └── Tailwind/                ← Tailwind CSVs + scheduling strategy
├── Products/
...
│   ├── Picture Book Companions/ (6 titles — All the Way to the Top/ · A Friend for Henry/ · I Talk Like a River/ · Ian's Walk/ · Emmanuel's Dream/ · My Friend Isabelle/ — each title: build_[title].js + build_symbol_pages_picbook.py; shared: build_student_complete.py; PDF: LibreOffice headless)
...
└── _Operations/
    ...
    ├── _Shared_Inserts/         ← Shared assets used across ALL product lines. Never store copies in unit folders. Includes: About/Accessibility/Terms docx (never rewrite) + AAC_Communication_Session_Tracker.pdf (authoritative copy — pulled by build_student_complete.py)
    ...
    │   └── UFLI_QC_Evaluation_Rubric.md            ← stays here (shared template)
    │   NOTE: Per-product QC records (diagnostics, content extractions, evaluations for ONE unit)
    │         belong in that unit's Product Files/ — NOT here.

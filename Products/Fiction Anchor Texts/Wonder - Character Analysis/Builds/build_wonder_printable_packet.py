...
SYMBOL_FILE = {
    ...
    "young":    "arasaac_little.png",        # ★ substitute — arasaac_young not in cache
    "old":      "arasaac_old.png",
    "carries":  "arasaac_carry.png",
    "runs":     "arasaac_run.png",
    "hides":    "arasaac_away.png",          # ★ substitute — arasaac_hide not in cache
...
def build_layer1_pdf() -> bytes:
    ...
    tools_compact = [
        ("p.2", "Core Word Symbol Cards (12)",
         "Core — SDI targets; likely already on the student's system"),
        ("p.3", "Fringe Word Symbol Cards (12)",
         "Fringe — SDI targets; include in communication packet"),
...
def _draw_board_cell_canvas(c, x, y, w, h, word, fk_color, sym_file=None,
    ...
    # White background — print-safe, FK color as border only
...
STUDENT_PARTS = [
    {
        ...
        "prompt": (
            "Choose a character from Wonder. Describe them. "
            "Draw the character or write describing words in the box."
        ),
...
def _draw_vocab_chip_canvas(c, x, y, word, chip_w, chip_h):
    """Draw a single Fitzgerald Key vocabulary chip — print-safe: white fill, FK border only."""
    fk_col = fk(word)
...
def _draw_student_page(c, PG_W, PG_H, part):
    ...
    if resp_type == "draw":
        ...
        c.setLineWidth(2.0)
        c.rect(M_X, y - 20, col_w, 20, fill=1, stroke=1)
        c.setFillColor(NAVY)
        c.setFont("Helvetica-Bold", 10)
        ...
        c.setStrokeColor(TEAL)
        c.setLineWidth(2.0)
        c.rect(M_X + col_w + 4, y - 20, col_w, 20, fill=1, stroke=1)
        c.setFillColor(NAVY)

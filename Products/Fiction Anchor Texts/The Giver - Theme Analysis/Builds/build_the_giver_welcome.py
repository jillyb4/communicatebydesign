"""
build_the_giver_welcome.py — Communicate by Design
====================================================
Builds the Welcome to the Product PDF for:
  The Giver: Theme Analysis — Fiction Anchor Text Unit
  Unit 2 of 6  |  Grades 6–8  |  RL.6.2 / RL.7.2 / RL.8.2

OUTPUT:
  The_Giver_Theme_Analysis_Welcome_to_the_Product.pdf

USAGE:
  python3 build_the_giver_welcome.py
"""
...
WELCOME_OUT = os.path.join(HERE, "The_Giver_Theme_Analysis_Welcome_to_the_Product.pdf")

UNIT_TITLE  = "The Giver: Theme Analysis"
GRADE_RANGE = "Grades 6–8"
STANDARDS   = "RL.6.2 / RL.7.2 / RL.8.2"
UNIT_NUM    = "Fiction Anchor Text Unit 2 of 6"


def build_welcome_pdf() -> bytes:
    """
    2-page branded Welcome to the Product PDF for The Giver.
    ...
    story = [
        ...
        Paragraph(
            "• <b>Teaching Materials.docx</b> — Full unit teacher reference: research base, novel overview, "
            "all 5 parts with activities and partner guidance at point of use, "
            "communication access vocabulary table, before-reading activities, Theme Evidence Chart "
            "template with sentence frames, IEP goal stems for RL.6.2/7.2/8.2, two AAC communication "
            "goals, rubric, and answer key. Convert to Google Docs for digital student access.", item),
        Paragraph(
            "• <b>Fiction Printable Packet (PDF, 13 pages)</b> — Communication Environment Setup guide · "
            "Core word symbol cards (12 words) · Fringe word symbol cards (12 words, chapter order) · "
            "Board A: Theme Evidence Vocabulary (landscape) · Board B: Emotion + Reasoning · "
            "Board C: Literary Discussion Moves · Vocabulary Map (14 core + 12 fringe) · "
        ...
        HRFlowable(width="100%", thickness=0.75, color=AMBER, spaceAfter=5),
        Paragraph(
            "Your students are ready to read <i>The Giver</i>. This unit gives them access "
            "to what it means — including students who communicate with AAC "
            "and students who have never been asked to do theme analysis before.",
            body),
        Paragraph(
            f"• <b>Whole-book theme analysis. ({STANDARDS})</b> — "
            "Five parts, one central question: <i>What makes us fully human — and what happens "
            "when those things are taken away?</i> Students build a full theme statement by Part 5. "
            "The Theme Evidence Chart is the scaffold; by the synthesis, it becomes the outline.",
            item),
        Paragraph(
            "• <b>The Theme Evidence Chart</b> — one chart built across all five parts, capturing "
            "what happens → what it shows → how it connects to the theme. No margin annotation "
            "codes to manage. By the synthesis, the chart is the outline. "
        ...
        Paragraph(
            ...
            "Every part has a clear AAC pathway: device, e-tran board, symbol cards, gaze access, "
            "or alternative pencil. Partner guidance is written at point of use, not in a separate "
            "supplement. Board A (Theme Evidence Vocabulary) is built specifically for this unit.",
            item),
        Paragraph(
            "• <b>AAC-inclusive by design — same text, same standard, different access layer.</b> "
            "Every student reads <i>The Giver</i>. The communication access tools — symbol cards, "
            "communication boards, partner guidance, and the Theme Evidence Chart — "
            "are what make the text reachable. The scaffold varies. The expectation does not.",
            item),
        Paragraph(
            "• <b>IEP-aligned by design</b> — includes model goal stems for RL.6.2/7.2/8.2 "
            "and two AAC communication goals. SDI component labeling built into teacher pages. "
        ...
        Paragraph(
            ...
            "The fringe word list (in chapter order) is on the symbol card page inside the Printable Packet. "
            "Core words are likely already on student systems — confirm before Part 1.",
            item),
        Paragraph(
            "<b>Step 2 — Run the Vocabulary Preview Routine before Part 1.</b> "
            "The routine in Teaching Materials.docx introduces the five highest-priority theme words "
            "using Say &amp; Show → Check → Connect → Flag. "
            "Pre-teaching these words is the single highest-leverage move for AAC-using students "
            "before any theme analysis task.",
            item),
        Paragraph(
            "<b>Step 3 — Set up communication access for each student.</b> "
            "Every student reads the same text. The access layer is the AAC support — "
            "confirm which communication boards, symbol cards, and response methods "
            "each student will use before Part 1 begins.",
            item),
        Paragraph(
            "<b>Step 4 — Print the Printable Packet.</b> "
            "Laminate the three communication boards (Board A/B/C) for repeated use. "
            "Print symbol cards as a class set or individual sets. "
            "Print one student response page per student per part of the book.",
            item),
        Paragraph(
            "<b>Step 5 — Build the Theme Evidence Chart across all five parts.</b> "
            "The chart is cumulative — students add to it each part. "
            "By Part 5 (Whole-Book Synthesis), the completed chart is the outline for "
            "the theme statement response. Partner guidance at each part tells teams "
            "exactly how to support this process.",
            item),
        Paragraph(
            "<b>Step 6 — Use the AAC Session Tracker.</b> "
        ...
        Paragraph(
            ...
            "AAC communication boards for literary discussion and theme analysis, "
...
def main():
    ...
    print(f"  2. Assemble TPT folder:")
    print(f"     • The_Giver_Theme_Analysis_Teaching Materials.docx")
    print(f"     • The_Giver_Theme_Analysis_Printable_Packet.pdf  (13pp)")
    print(f"     • The_Giver_Theme_Analysis_Welcome_to_the_Product.pdf  (2pp)")
    print(f"  3. Export Teaching Materials.docx → PDF via Word (File → Save As → PDF)")
    print(f"  4. Update Airtable Products record for The Giver")
    print(f"{'═'*60}\n")
